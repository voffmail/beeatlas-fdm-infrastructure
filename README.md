...
* `docker compose build vscode; docker compose pull; docker compose up --no-build -d`
будет запускать готовые образы (кроме vscode) из image; рекомендован для быстрого запуска;


* `docker compose up --build -d`
произведёт сборку большинства компонент, кроме 3rd-party и запустит сденд; рекомендован для локальной разработки
...
Значения для `gateway` — в **`docker-compose.yml`** (секция `gateway` → `environment`). Подробная таблица — [раздел 8](#8-конфигурация). После смены режима: `docker compose up -d gateway`.
...
Переменные задаются в **`docker-compose.yml`** (секция `gateway` → `environment`).  
...
- **`docker-compose.yml`** — режим gateway, порты, переменные frontend
